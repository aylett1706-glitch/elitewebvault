...
export default function AdminAnalyticsDashboard() {
  ...
  useEffect(() => { Promise.all([supabaseApi.entities.Media.list('-views', 300), supabaseApi.entities.Game.list('-views', 100), supabaseApi.entities.WatchHistory.list('-last_watched', 300)]).then(([m,g,h]) => { setMedia(m); setGames(g); setHistory(h); }); }, []);
